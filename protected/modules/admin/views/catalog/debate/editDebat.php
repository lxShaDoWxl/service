...
<div class="scrollable">
	...
	<div class="tabs" id="tabs-1">
		<div class="tab active" id="tab-1">
			<form method="post" action=" <?= CHtml::normalizeUrl(array('catalog/saveDebat','id'=>$debat->id))?>" class="modal">
				<table class="step-right">
					<tr>
						<td><strong>Название:</strong></td>
						<td><input class="input wide" name="title" type="text" required id="title" value="<?=$debat->title?>"></td>
						<td></td>
					</tr>
					<tr>
						<td><strong>Теги:</strong></td>
						<td><input class="input wide" name="tag" type="text" id="title" value="<?=$debat->tag?>"></td>
						<td></td>
					</tr>
					<tr>
						...
						<td>
							<select class="switch" name="isVisible">
								<option value="0" <?=($debat->isVisible==0)?'selected':''?>>1</option>
								<option value="1" <?=($debat->isVisible==1)?'selected':''?>>0</option>
							</select>
						</td>
					...
					<tr>
						...
						<td><input class="input wide" name="date_start" type="date" required id="date" value="<?=$debat->date_start?>"></td>
						<td></td>
					</tr>
					<tr>
						<td><strong>Дата конца:</strong></td>
						<td><input class="input wide" name="date_end" type="date" required id="date" value="<?=$debat->date_end?>"></td>
						<td></td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="3">
							<textarea name="body" class="input wide"><?=$debat->body?></textarea>
						</td>
					</tr>
					<tr>
					</tr>
		...
		<div class="tab" id="tab-2">
			<div class="step-right">
				...
				<div class="images images-sortable">
					<div class="image">
						<div class="icon100"><img src="<?= $this->baseUrl . $debat->img ?>" class="round100"></div>
						<div class="image-remove"> <a class="btn-red delete-image" data-id="<?= $debat->id ?>">Удалить</a> </div>
					</div>
				</div>

				<h4>Добавить изображения:</h4>
				<div class="image-drop dropzone"></div>
				<input class="input" name="img" type="hidden" id="img_url" value="<?=$debat->img?>">
			</div>
		</div>
					...
					<td>
						<select name="experts[]" class='items'   multiple>
							<? foreach ($experts as $item): ?>
								<? if ($item->connectDebats(array('condition'=>'`id_debat`=:id','params'=>array(':id'=>$debat->id)))): ?>
