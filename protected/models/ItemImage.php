...
class ItemImage extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('desc, title, item_id, position, img_url', 'required'),
			array('item_id, position', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>255),
			array('img_url', 'length', 'max'=>500),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, desc, title, item_id, position, img_url', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'item' => array(self::BELONGS_TO, 'Catalog', 'item_id'),
		);
	}
