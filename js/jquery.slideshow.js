...
function initSlideShow() {
	jQuery('div.slider_wr').fadeGallery({
		slides: '.slide',
        activeClass:'current',
        btnPrev: '.sl_n_left',
        btnNext: '.sl_n_right',
		...
		generatePagination: 'ul.slider_pagination',
		autoRotation: true,
		autoHeight: false,
        pauseOnHover: true,
	...
	jQuery('div.project-slide').fadeGallery({
		slides: '.gmask li',
		currentNumber: 'span.cur-num',
		totalNumber: 'span.all-num',
		switchSimultaneously: true,
	...
	FadeGallery.prototype = {
		...
		switchSlide: function() {
			...
			if(this.slides.length > 1) {
				...
				this.slides.eq(this.prevIndex).stop().animate({display:'none', opacity:0},{duration: this.options.animSpeed, complete: function(){
					self.slides.eq(self.prevIndex).css({opacity:0, display:'none'});
				}});
				clearTimeout(this.switchTimer);
				this.switchTimer = setTimeout(function(){
					self.slides.eq(self.currentIndex).stop().animate({display:'block', opacity:1 },{duration: self.options.animSpeed, complete: function(){
						self.slides.eq(self.currentIndex).css({display:'block', opacity:''});
		...
		refreshState: function(initial) {
			...
			if(this.options.autoHeight) {
				if(initial) {
					this.slidesHolder.css({height: (this.slides.eq(this.currentIndex).outerHeight())});
				} else {
